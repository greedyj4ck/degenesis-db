...
game.importDataAmmo= function() {
let weapons = []
fetch("systems/degenesis/import/ammunition.json").then(r => r.json().then(json => {
for (let item of json.items)
{
  let itemData = duplicate(game.system.model.Item.ammunition)
  
  
  itemData.description = item.description
  itemData.damage = item.damage || 0 
  itemData.tech = item.technology || 0
  itemData.value = item.price || 0
 ...
 /*  itemData.qualities = item.qualities.map(q => {
  ...
  let newItem = {name : item.ammo, data : itemData, type : "ammunition"}
